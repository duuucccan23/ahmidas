...
namespace Contract
{
                                                          ...
                                                          size_t const t_src, size_t const t_snk)
  {
    ...
    for (size_t opNo=0; opNo<ops.size(); opNo++)
    {
      ...
      //Core::Propagator::const_iterator It_xi(xi_at_sink.begin());
        ...
        for(size_t idx_X=0; idx_X<L; idx_X++)
        {
          ...
          for(size_t idx_Y=0; idx_Y<L; idx_Y++)
          {
            ...
            for(size_t idx_Z=0; idx_Z<L; idx_Z++)
            {
              ...
              QCD::Tensor xi_u_snk(xi_at_sink(pos_snk)*gamma5);
              QCD::Tensor xi_d_snk(xi_u_snk);

              QCD::Tensor tmp_d_from_source(*It_d);
              ...
              //QCD::Tensor tmp_u1(*It_u1);
              //QCD::Tensor tmp_u2(*It_u2);

              tmp_d_from_sink.conjugate();
              tmp_d_from_sink*=gamma5;
              tmp_d_from_sink.rightMultiply(xi_d_snk);
              ...
              getDiracMatrix(dd_tmp, S_u1_xf, tmp_d_from_sink, S_u2_xf, Base::bar_PROTON);
              (*It_dd)+= dd_tmp;
            }
          }
        }
