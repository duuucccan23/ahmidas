...
int main(int argc, char **argv)
{
    ...
    bool const debug=bool(floats["debug"] != 0.0); // 0=no,!=0 =yes
	...
	if (Nsample%12 != 0){ 
		...
		if(weave.isRoot())	
			std::cout << "Nsample must be a multiple of 12 ... exiting !\n" << std::endl;
		...
		for (size_t n=0;n<N;n++)
		{
			...
			if(debug==true)
			{
				...
				xi = phi_vv.applyDiracOperator(gauge_field,kappa,mu,thetat,thetax,thetay,thetaz,Base::Full); 
				...
				double const xiNorm_vv = xi.norm();
				if (weave.isRoot())
					std::cout << "norm of source calculated from  vvSum propagator: " << xiNorm_vv << std::endl;

				double const phiNorm_vv = phi_vv.norm();
				if (weave.isRoot())
					std::cout << "norm of the vvSum propagator: " << phiNorm_vv << std::endl;
			...
			std::vector< std::complex <double>  > C_vv2 = Contract::compute_loop_new(g5_phi,phi_vv,my_operators);
			...
			for(size_t i=0; i < C_vv.size(); i++)
			{
				...
				C_vv2[i] *=  std::complex<double>(2.0/(16.*kappa*kappa),0);	 // factor = + 2 /( 4 kappa) ^2
			}
			...
			if (weave.isRoot())	
			{
				...
				if (n==0)
				{
					...
					fout_vv=fopen("output_disc_vvSum.dat","w");
					fout_vv2=fopen("output_disc_vvSum2.dat","w");
					//	fout_vv_sum.open("output_disc_vv_sum.dat");

#ifdef _with_momentum_projection
					fout_vv_mom=fopen("output_disc_vvSum_mom.dat","w");
#endif
					fout_conserved_vv=fopen("output_disc_conserved_vvSum.dat","w");

#ifdef _with_Omunu_
					fout_twist2_vv=fopen("output_disc_twist2_vvSum.dat","w");
					fout_twist2_pol_vv=fopen("output_disc_twist2_pol_vvSum.dat","w");
				...
				else	
				{
					...
					fout_vv=fopen("output_disc_vvSum.dat","a");
					fout_vv2=fopen("output_disc_vvSum2.dat","a");

#ifdef _with_momentum_projection
					fout_vv_mom=fopen("output_disc_vvSum_mom.dat","a");
#endif
					fout_conserved_vv=fopen("output_disc_conserved_vvSum.dat","a");

#ifdef _with_Omunu_
					fout_twist2_vv=fopen("output_disc_twist2_vvSum.dat","a");
					fout_twist2_pol_vv=fopen("output_disc_twist2_pol_vvSum.dat","a");
				...
				for(size_t j=0; j<12; j++)
				{
					...
					for(size_t i=0; i<my_operators.size(); i++)
					{
						...
						for(size_t t = 0; t < T; t++)
						{
							...
							fprintf(fout_vv2,"%3d %3d %3d %3.10e %3.10e\n",t,i,j+12*n, C_vv2[t +  T*i + 16*T*j].real(),C_vv2[t + T*i + 16*T*j].imag() );
