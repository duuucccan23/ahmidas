...
/*
  This code loads two forward propagators (u,d) and creates the sources of 8 backward propagators (using
  4 different projectors for both u and d current).
  Use some inverter, preferrably the tmLQCD one by ETMC (Urbach et al.) to calculate the backward propagators.
  Then, use contract_nucleon3point to calculate whatever 3-point correlation function is desired.
  Some modifications might have to be done there concerning which correlation function is calculated and
...
int main(int argc, char **argv)
{
  ...
  Input::FileReader reader("./generate_sequential_sources_nucleon_input.xml");
  ...
  std::vector< std::string > const &seqSourceFilesD4(files[ 3]);
  std::vector< std::string > const &seqSourceFilesU4(files[ 4]);
  std::vector< std::string > const &seqSourceFilesD1(files[ 5]);
  std::vector< std::string > const &seqSourceFilesU1(files[ 6]);
  std::vector< std::string > const &seqSourceFilesD2(files[ 7]);
  std::vector< std::string > const &seqSourceFilesU2(files[ 8]);
  std::vector< std::string > const &seqSourceFilesD3(files[ 9]);
  std::vector< std::string > const &seqSourceFilesU3(files[10]);
  ...
  Contract::create_sequential_source_proton_d(sequentialSource1, *uProp, *uProp,
                                              gauge_field, Smear::sm_Jacobi, Jac_iterations, Jac_alpha,
                                              timeslice_sink, Base::proj_1__MINUS_TM);
